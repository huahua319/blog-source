...
1. 本文件（快速约束）
2. [CLAUDE_LEARNINGS.md](CLAUDE_LEARNINGS.md)（**必读**，NexT / Hexo 项目踩坑与避坑指南：样式定制、缓存、本地预览工作流等）
3. [SPEC.md](SPEC.md) §1–§9（完整工程说明，**必读**）
4. 按任务深入：
   - 写 / 改文章 → SPEC §6 + [USAGE.md](USAGE.md)
   - 改配置 / 主题 → SPEC §5 + §7
   - 动依赖 / 构建 → SPEC §3 + §8
   - 任何修改前 → SPEC §9（11 条修改原则）
...
- 文档与实际不一致时，**以实际文件为准**
- 改完自动在后台启动 `npm run dev` 本地预览；**不主动 `hexo deploy`**，部署需作者明确指示（§8.5）
